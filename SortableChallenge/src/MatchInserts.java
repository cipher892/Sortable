...
public class MatchInserts 
{
	...
	private static Map<String,List<JSONObject>> productListings = new HashMap<>();
		...
		if(file.exists())
		{
			try
			{
				...
				while (input.hasNextLine())
				{
					...
					matchListings(maker, title, tokens, jsonobj);
	...
	private static void matchListings(String maker, String title, String[] tokens, JSONObject jsonobj)
	{
...
//method that adds listings to the productLisitngs map 
  private static void addListing(String product, JSONObject jsonobj)
  {
	 if(!product.equalsIgnoreCase(" "))
		{
		   ...
		   if(productListings.containsKey(product))
			 {
				List<JSONObject> listings = productListings.get(product);
				listings.add(jsonobj);
	...
	private static void displayResult(Map<String, List<JSONObject>> arr)
	{
		...
		for(String key : keys)
		{
			List<JSONObject> list = arr.get(key);
			JSONObject jsonobj = new JSONObject();
			jsonobj.put("product_name", key);
			jsonobj.put("listings", list);
